...
SUPPORTED_ENTITIES = ['IBAN_CODE', 'US_PASSPORT', 'DATE_TIME', 'MEDICAL_LICENSE', 'CRYPTO', 'LOCATION', 'UK_NHS', 'US_SSN', 'CREDIT_CARD', 'US_BANK_NUMBER', 'US_ITIN', 'EMAIL_ADDRESS', 'PERSON', 'IP_ADDRESS', 'DOMAIN_NAME', 'PHONE_NUMBER', 'SG_NRIC_FIN', 'NRP', 'US_DRIVER_LICENSE','GEO_COORDINATES']
ANONYMIZERS = ['hash', 'mask', 'redact', 'replace', 'custom', 'encrypt', 'decrypt','geocoordinates']
...
class ClientEntity:
    ...
    def sendRequestAnonymize(self, filename):
        ...
        try:
            ...
            chunk_iterator = generateChunks(path + filename)
    ...
    def sendRequestDeanonymize(self, filename):
        ...
        path = PATH_ANONYMIZER_RESULTS
        
        if("pira" in filename):
            path = PATH_ANONYMIZER_RESULTS + "pira_results/"
    ...
    def sendRequestForText(self, filename, uuidClient, requestType):
        ...
        if requestType == "anonymize":
            with open(path + filename + "-anonymized.txt", "w") as AnonymizerResults:
...
def readRecognizerResults(filename, uuidClient):
    ...
    path = PATH_ANALYZER_RESULTS
        
    if("pira" in filename):
        path = PATH_ANALYZER_RESULTS + "pira_results/"

    print(path)

    with open(path + filename + "-results.txt", "r") as resultsFile:
...
def readAnonymizedItems(filename, uuidClient):
    ...
    with open(path + filename + "-items.txt", "r") as itemsFile:
...
def checkRequiredFiles(filename, requestType):
    ...
    path_analyzer = PATH_ANALYZER_RESULTS


    if("pira" in filename):
        path_analyzer = PATH_ANALYZER_RESULTS + "pira_results/"
        path_files = PATH_FILES + "pira_files/"

    if requestType == "anonymize":
        
        # check on files
        if (not os.path.exists(path_files + filename + ".txt")) or (not os.path.exists(path_analyzer + filename + "-results.txt")):
...
def addGeocoordinates(entity_type, function, radius, radius2,variance, location2, variance2):
    # check for duplicate
    found = checkDuplicate(entity_type, CONFIG_FILE)
    params = '{ ' + f'\\"type\\": \\"geocoordinates\\" , \\"function\\": \\"{function}\\", \\"radius\\" : {float(radius)}, \\"radius2\\":{float(radius2)} , \\"variance\\":{float(variance)}, \\"location2\\":\\"{location2}\\",\\"variance2\\":{float(variance2)}' + ' }'
